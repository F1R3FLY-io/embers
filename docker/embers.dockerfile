...
RUN apt-get update && \
    apt-get install -y pkg-config protobuf-compiler libssl-dev cmake clang gcc-aarch64-linux-gnu libc6-dev-arm64-cross && \
    rustup toolchain install nightly-2026-02-09 && \
    rustup target add x86_64-unknown-linux-gnu --toolchain nightly-2026-02-09 && \
    rustup target add aarch64-unknown-linux-gnu --toolchain nightly-2026-02-09
...
RUN \
    ...
    RUSTFLAGS="-C target-feature=+aes,+neon" cargo build --release --target aarch64-unknown-linux-gnu && \
