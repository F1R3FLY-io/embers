...
class UpdateResponce:
    ...
    def wait_for_sync(self) -> Self:
        self._accepted.wait(timeout=15)
        return self
...
class TestnetApi:
    ...
    def deploy(self, wallet: Wallet, test: str, env: str | None = None) -> UpdateResponce:
        ...
        return UpdateResponce(
            first=resp,
            second=resp_next,
            accepted=self._client.api_sync.register(resp_next.json["deployer_id"]),
        )
...
class AiAgentsTeamsApi:
    ...
    def run(self, wallet: Wallet, prompt: str, phlo_limit: int, agents_team: str) -> UpdateResponce:
        resp = self._client.post(
