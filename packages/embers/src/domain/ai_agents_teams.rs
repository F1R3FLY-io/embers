...
use crate::domain::ai_agents_teams::models::FireskyCredentials;
...
impl AgentsTeamsService {
    ...
    pub async fn bootstrap(
        ...
        let encrypted_accounts: Result<Vec<blockchain::ai_agents_teams::models::EncryptedMsg>, _> =
            read_client.get_data(code).await;

        let firesky_accounts =
            match encrypted_accounts {
                Ok(encrypted_accounts) => encrypted_accounts
                    .into_iter()
                    .map(|account| {
                        deserialize_decrypted::<
                            blockchain::ai_agents_teams::models::FireskyCredentials,
                        >(account.into(), &aes_encryption_key)
                    })
                    .inspect(|decrypted| {
